...
Simply search for **docker ce with centos 8.2** and then click **Create**.
 ![Docker image](media/java-get-started-with-elasticsearch/docker-marketplace.png)
...
- `ELASTICSEARCH_PASSWORD` = the password of the user created by following the ***Grant user access to secured resources*** document referenced previously, or the password provided during deployment to be used with the `elastic` user.
...
> The network the VM is using will need a firewall rule applied to accept traffic on port 8000 for the OpBeans web application to be accessible from an external resource, such as your personal laptop. Please refer to the Azure documentation if unfamiliar with this process of creating a firewall rule.
 ![Elasticsearch endpoint](media/java-get-started-with-elasticsearch/elasticsearch-endpoint.png)

![APM Token](media/java-get-started-with-elasticsearch/apm-secret.png)
...
> [!NOTE]
> The process for setting up Filebeat is possible outside of using containers, simply by following the [Filebeat Quickstart Guide](https://www.elastic.co/guide/en/beats/filebeat/current/filebeat-installation-configuration.html).
...
Click **View in app**.
![View in app](media/java-get-started-with-elasticsearch/view-in-app.png)
...
![Log Histogram](media/java-get-started-with-elasticsearch/histogram.png)
...
![Anomalies](media/java-get-started-with-elasticsearch/anomalies.png)



Next, from the navigation menu click **Discover**.
![Discover](media/java-get-started-with-elasticsearch/discover.png)
...
![Filebeat container](media/java-get-started-with-elasticsearch/filebeat-container.png)
...
There are essentially three ways a Java developer can set up the Elastic APM Java Agent in their own environment. This tutorial's package attaches to the JVM as a standalone application using `apm-agent-attach-standalone.jar`. There have been no custom code snippets defined to tell APM what to do; no custom instrumentation has been performed. The solution used in this tutorial is completely out-of-the-box.
...
![APM](media/java-get-started-with-elasticsearch/apm.png)
...
### Invoking a bug
The OpBeans app comes with a built in bug to help highlight what may occur and how to begin to troubleshoot. Simply add `/is-it-coffee-time` to the end of the URL.
...
Now from within Kibana, click the **Errors** tab from the **opbeans-java** service page.
 ![Demo exception](media/java-get-started-with-elasticsearch/bug.png)
...
![View in Discover](media/java-get-started-with-elasticsearch/view-in-discover.png)
...
Another very helpful feature is the ability to navigate to the error identified with the documents that occurred within the same period of time. Simply click **View surrounding documents**.
